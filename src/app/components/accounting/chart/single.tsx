...
export default function SingleEntry(props: {
    ...
    const loadEntry = async() => {
        setLoading(true);
        const entryRef = ref(realtime, `accounting/chart/${props.id}`);
            ...
            <div className="max-w-2xl px-6 py-16 mx-auto space-y-12">
                <article className="space-y-8 text-gray-900">
                    <div className="space-y-6">
                        ...
                        <div className="flex flex-col items-start justify-between w-full md:flex-row md:items-center text-gray-600">
                            <div className="flex items-center md:space-x-2">
                                ...
                                <p className="text-sm">Leroy Jenkins • <time dateTime={entry?.createdDate}>{formatDate(new Date(entry?.createdDate || ""), "MM/DD/YYYY")}</time></p>
                            </div>
                    ...
                    <div className="text-gray-800">
                        ...
                        <p title="Entry Date">
                            Date: <time dateTime={entry?.date}>{formatDate(new Date(entry?.date || ""), "MM/DD/YYYY")}</time>
                        </p>
                    </div>
                </article>
                <div>
